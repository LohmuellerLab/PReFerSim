...
int BurnInPopSizeFlag = 0, SFSBurnInFlag = 0, SFSIntIntervalFlag = 0;
...
//	printf("%s\n",argv[1]);
	if (argc != 3){
		fprintf(stderr, "Error: The number of arguments given in the command line is not correct. Either the parameter file or the replicate number were not specified in the command line. General usage of the program: GSL_RNG_SEED=<seed> GSL_RNG_TYPE=mrg ./fwd_seldist_gsl_2012_4epoch.debug <Parameter file> <replicate number>\n");
		exit(8);
	}
...
/*	NeTrajFile = fopen (ParameterFile,"r");
	...
	if (PARFILE == NULL)
    {
		fprintf(stderr, "Error: Unable to read parameter file\n");
		exit(8);
	}
...
//								SubString3[strlen(SubString3) -1] = '\0';
					...
					for (j=0; j<strlen(ParSubString); j++) {
						...
						// && strcmp(SingleCharacter,".")!=0
						if (!isdigit((int) SingleCharacter[0]) ){
							fprintf(stderr, "\nError: The value %s given in the option LastGenerationAFSamplingValue is not an integer number equal to 0 or 1 (no dots or commas allowed)\n", ParSubString);
							exit(8);
						}
					}
...
//					SampleFlag = 1;
					...
					F_fixedflag = 1;
					if ( (F < 0.0) || (F> 1.0 ) ) {
						fprintf(stderr, "\nError: The value %lf given in the option FixedFValue is not bounded between 0 and 1\n", F);
						exit(8);
					}
...
//							F_Epochs[NumberOfEpochs_F] = atoi(pch);
					...
					while (fscanf(FValuesFile, "%lf",&F_Temp) != EOF){
						...
						if ( (F_Epochs[NumberOfEpochs_F] < 0) || (F_Epochs[NumberOfEpochs_F]>1) ) {
							fprintf(stderr, "\nError: The value %lf from the epoch number %d given in the ChangedFValue file is not bounded between 0 and 1\n", F_Epochs[NumberOfEpochs_F],NumberOfEpochs_F);
							exit(8);
						}
...
//					printf("Make it here? %d\n", NumberOfEpochs_F);
					...
					printf("%lf\n",h);
					if ( (h< 0.0) || (h>1.0) ) {
						fprintf(stderr, "\nError: The value %lf given in the option h is not bounded between 0 and 1\n", h);
						exit(8);
					}
				}
					...
					for (j=0; j<strlen(ParSubString); j++) {
						...
						// && strcmp(SingleCharacter,".")!=0
						if (!isdigit((int) SingleCharacter[0]) ){
							fprintf(stderr, "\nError: The value %s given in the option SFSNoBurnIn is not an integer number equal to 0 or 1 (no dots or commas allowed)\n", ParSubString);
							exit(8);
						}
					}					
					BurnInPopSizeFlag = atoi(ParSubString) ;
					printf("%d\n",BurnInPopSizeFlag);
					if (BurnInPopSizeFlag != 0 && BurnInPopSizeFlag != 1){
						fprintf(stderr, "\nError: The value %s given in the option SFSNoBurnIn is not an integer number equal to 0 or 1 (no dots or commas allowed)\n", ParSubString);
						exit(8);						
					}
				...
				else if (strcasecmp(ParameterFileArgument,"PrintSumDAF:") == 0){
					if (PrintAverageDAFFlag == 1){
						fprintf(stderr, "\nError: At least two occurences of the option PrintSumDAF in the parameter file. Use that option only once in the parameter file.\n");
						exit(8);
					}
					...
					for (j=0; j<strlen(ParSubString); j++) {
						...
						// && strcmp(SingleCharacter,".")!=0
						if (!isdigit((int) SingleCharacter[0]) ){
							fprintf(stderr, "\nError: The value %s given in the option PrintSumDAF is not an integer number equal to 0 or 1 (no dots or commas allowed)\n", ParSubString);
							exit(8);
						}
					}
...
//	fprintf(stderr,"%s\n",path);
	...
	if ( (TotalRelaxationParameters > 0) && ( TotalRelaxationParameters < 4) ){
		fprintf(stderr, "Error: To relax the selection coefficient, you must add four parameters into the parameter file: EpochOfRelaxation, RelaxationSelectionThreshold, RelaxationSelectionType and RelaxationSelectionCoefficient.\n");
		exit(8);
...
printf("Ne = %i\tgenerations = %i\t", N[e], gen[e]);
	ProjectedNumberOfMutations = ProjectedNumberOfMutations + mut_rate * N[e]/N[0] * gen[e];
if (F_fixedflag == 1){
F_Epochs[e] = F;
}
	printf("F = %lf\n",F_Epochs[e]);
	...
	printf("\n");
	if (ProjectedNumberOfMutations > 2000000000.0){
		fprintf(stderr, "Error: The average projected number of mutations with the mutation rate you gave is %lf. Please reduce the value given in MutationRate, the average number of mutations should not be bigger than 2000000000.0\n",ProjectedNumberOfMutations);
		exit(8);
	}
//printf("Here?\n");
// return 0;
...
char file_out_maf[1000];
 sprintf(file_out_maf, "%s.%d.sum_daf_out.txt",OutputFilePrefix,num);
  //FILE *MAF_OUT;
...
//now for genetic load
	...
	//now create genotype sampled output
	FILE *DOG_OUT;
	if (het_outFlag == 1) {
		char file_out_dog[1000];
		sprintf(file_out_dog, "%s.%d.sampledgenotype_out.txt",OutputFilePrefix,num);
		DOG_OUT = fopen(file_out_dog, "w"); //output file for genotype sampled stuff
		if (DOG_OUT == NULL)
		{
			fprintf(stderr, "Error: unable to create output file %s\n",file_out_dog);
			exit(8);
...
//note, here we can look at 4 epochs!
	if ((BurnInPopSizeFlag==1) && ((strcasecmp(type_sel,"point") != 0))){
		fprintf(stderr, "Error: If you use the SFSNoBurnIn option, you must use the DFEType: point option and specify a selection coefficient value with the option DFEPointSelectionCoefficient\n");
		exit(8);
	}
	if ((BurnInPopSizeFlag==1) && (h != 0.5)){
		fprintf(stderr, "Error: If you use the SFSNoBurnIn option, you must use a h value equal to 0.5\n");
		exit(8);
	}

	if ((BurnInPopSizeFlag==1) && (AlleleTrajsInputFlag == 1)){
		fprintf(stderr, "Error: If you use the SFSNoBurnIn option, you cannot use the AlleleTrajsInput option\n");
		exit(8);
	}
	
	if ((BurnInPopSizeFlag==1) && (AlleleTrajsOutputFlag == 1)){
		fprintf(stderr, "Error: If you use the SFSNoBurnIn option, you cannot use the AlleleTrajsOutput option\n");
		exit(8);
	}
	
	if ((BurnInPopSizeFlag==1) && ((strcasecmp(type_sel,"point") == 0)) && (h == 0.5)){

	TheoreticalSFS = malloc(N[0]*sizeof(double));
	...
	BurnInPopSizeInIntegrableFunction = N[0];
		if (PointSel > 0.5){
			printf("WARNING: The value given in DFEPointSelectionCoefficient was bigger than 0.5. This value has been changed to 0.5\n");
			PointSel = 0.5;
		}
	PointSelInIntegrableFunction = PointSel;
	SampleSizeInIntegrableFunction = N[0];
	ThetaInIntegrableFunction = mut_rate * 2;
	
	for (jInIntegrableFunction = 1; jInIntegrableFunction< N[0]; jInIntegrableFunction++){
		gsl_integration_qags (&IntegrableFunction, 0, 1, 0, 1e-9, 1000, w, &result, &error); 
		TheoreticalSFS[jInIntegrableFunction-1] = result;
		SFSBasedAlleleFrequency = (float) jInIntegrableFunction / (float) N[0];
...
//		printf ("%d\t%.18f\t%lf\n", jInIntegrableFunction, TheoreticalSFS[jInIntegrableFunction-1], NumberOfMutationsToAdd);
	// struct linked_list *add_mutation_BurnIn(struct linked_list *head_ptr, double mut_rate, int N, gsl_rng * r, int age, char *type_sel, double sel_mult, double h , double PointSel, int *ListOfAllelesToKeep, int FlagTrajToPrint, FILE *EXITTRAJFILE , int FlagRelaxationSel, double SelThreshold, double NewSelCoef , double ParamOne, double ParamTwo,  double ParamThree, int TypeRelaxedSelection, double *VecSelParamOne, double *VecSelParamTwo, double *VecSelParamThree, int NumberOfSelPars, double SFSBasedAlleleFrequency);
	first_ptr = add_mutation_BurnIn(first_ptr,NumberOfMutationsToAdd, N[0], r, -1, type_sel, sel_mult, h, PointSel, ListOfAllelesToKeep, FlagTrajToPrint, EXITTRAJFILE, 0, SelectionThreshold, NewSelectionCoefficient, DistParamOne, DistParamTwo, DistParamThree, TypeOfRelaxedSelection, VectorSelParamOne , VectorSelParamTwo , VectorSelParamThree, NumberOfSelParameters, SFSBasedAlleleFrequency);	
...
/*	if (SampleFlag == 1){
		first_ptr = drift_sel(first_ptr,n, r, F_Epochs[e-1], FlagTrajToPrint, ListOfAllelesToKeep, EXITTRAJFILE, FIXEDSITESFILE ,Short, SampleFlag, 1, 0);
	} */
	
	if (full_outFlag == 1 && SampleFlag == 0){
...
//	printf("Current sel = %f %f %i %f\n", VecSelParamOne[SelIterator], randomvar, SelIterator, VecSelParamTwo[SelIterator]);
	...
	}else if (sel > CurSelThreshold && TypeRelaxedSelection == 0) {
		sel = sel * 2.0;
	}else if (sel > CurSelThreshold && TypeRelaxedSelection == 1) {
		sel = sel * 2.0;
	}
...
struct linked_list *current_ptr;
	...
	if(pop_freq<1.0 && pop_freq>0.)
	{
		s_to_print = s / 2;
		age_to_print = age + 1;
       		fprintf(FULL_OUT, "%d\t%e\t%e\t%d\t%d\n",num, pop_freq,s_to_print,age_to_print,counter); 
	}
...
//    printf("Here");
	...
	{  
		...
		if ( (full_outFlag == 1 ) && ( SampleFlag == 1 ) ){
			s_to_print = s / 2;
			age_to_print = age + 1;
...

{
	...
	double prob_1=0.0;
	double prob_2=0.0;
	unsigned int geno_1[3];
	unsigned int geno_2[3];
...
struct linked_list *current_ptr;
 ...
 while (current_ptr != NULL)
	...
	fprintf(DOG_OUT, "%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n",num, count_hom_anc_1,count_het_1,count_hom_der_1,count_hom_anc_2,count_het_2,count_hom_der_2,count_het_both);
